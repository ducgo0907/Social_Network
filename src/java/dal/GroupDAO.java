...
public class GroupDAO extends DBContext {
    ...
    public void acceptMember(int memberId, int groupId) {
        ...
        String sql = "Update JoinGroup set status = 'accepted', dateJoin = ?"
    ...
    public List<Group> getAllGroupByName(String name) {
        List<Group> list = new ArrayList<>();
        String sql = "Select * from Groups where name like ? ORDER BY groupid DESC";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, "%" + name + "%");
            ResultSet rs = st.executeQuery();
    ...
    public static void main(String[] args) {
        GroupDAO gd = new GroupDAO();
        System.out.println(gd.getGroupById(2).getOwnerId());
    }
